...
class QueryDoaminName(object):
    ...
    def __get_whois_list(self):
        ...
        :return list:
        """
        json_file_path = "whois.servers.json"
    ...
    def __match_info(self):
        ...
        if not match_status:
            match_status = re.search("Domain Name: " + self.domain_name, self.domain_info)
        if match_status:
            return True
        return False
    ...
    def __get_domain_info(self):
        ...
        for url in servers_list:
            ...
            try:
                ...
                code = "-3"
                logging.error("Connect [%s] error [%s], suffix [%s]"
                              % (url, e, self.domain_suffix))
            else:  # 连接服务器成功
                # 开始查询
...
def parserDomainName(domain_name):
    ...
    解析域名
    :param domain_name:
    :return (bool, str, (str, str)):  (bool, statusCode, (fld[完整域名], tld[域名后缀]))
    """
    try:
        res = get_tld(domain_name, as_object=True, fix_protocol=True)
    except Exception as e:
        logging.error("parser [%s] domain name has error -> [%s]" % (domain_name, e))
        return False, "-2", (None, None)
    logging.info("parser [%s] success: result [%s]" % (domain_name, res.fld))
    return True, None, (res.fld, res.tld)


def getDoaminWhois(domain_namem):
    """
    获取域名具体信息
...
def main():
    while not workQueue.empty():
        domain=workQueue.get(timeout=2)
        data=getDoaminWhois(domain)
        if data !=None:
            print(domain)
            result.append(data)
        else:
...
if __name__ == '__main__':
    ...
    jobs = []
    result=[]
    df = pd.read_csv('mal_domains.csv')
    for i in df['domain']:
        workQueue.put_nowait(i)
    for count in range(0,20):
        jobs.append(gevent.spawn(main()))
    gevent.joinall(jobs)
